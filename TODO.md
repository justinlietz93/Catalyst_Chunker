...
- [x] Step 1.3.2: Add contradiction fixtures for missing source IDs, broken offsets, and lossy normalization without declaration.
...
- [x] Step 2.2.5: Add contradiction fixtures for fake headings inside code fences and malformed Markdown regions.

### Task 2.3: Prepare observation extension points

- [x] Step 2.3.1: Define instrument contracts for table, PDF layout, code AST, and semantic shift observation.
- [x] Step 2.3.2: Keep external tools as instruments or boundary adapters, not internal truth models.
- [x] Step 2.3.3: Add tests that reject observations with no source reference.
...
- [x] Step 3.2.1: Implement source coverage checks.
- [x] Step 3.2.2: Implement source lineage checks.
- [x] Step 3.2.3: Implement offset reversibility checks.
...
- [x] Step 3.3.2: Enforce ADR-0003 by failing fixed-size fallback that lacks fallback evidence.
...
- [x] Step 4.2.2: Group by heading, paragraph, list, sentence, table, and token observations.
- [x] Step 4.2.3: Preserve source lineage for every candidate span.
- [x] Step 4.2.4: Add fixtures for very short paragraphs that should merge and very long paragraphs that must split safely.

### Task 4.3: Implement recursive and fixed-size fallback constraints from ADR-0003

- [x] Step 4.3.1: Implement `RecursiveFallbackCandidateSet`.
- [x] Step 4.3.2: Define the exact evidence that proves structural candidates failed or were unavailable.
- [x] Step 4.3.3: Permit fixed-size slicing only after fallback evidence is recorded.
- [x] Step 4.3.4: Emit audit-visible fallback records whenever fixed-size slicing is used.
...
- [x] Step 5.1.3: Add relation kinds for parent, child, previous, next, continuation, dependency, table, citation, code, summary, and retrieval-window relations.
...
- [x] Step 6.1.3: Implement `evaluate-candidates` to compare candidate sets without accepting hidden splitter output.
- [x] Step 6.1.4: Implement `emit-projection` for retrieval, audit, and boundary inspection outputs.
...
- [x] Step 6.2.3: Add contract tests for each port.
...
- [x] Step 6.3.3: Implement tokenizer adapter.
- [x] Step 6.3.4: Implement CLI presentation for `chunk`, `inspect-boundaries`, `compare-strategies`, `explain-chunk`, and `audit`.
...
- [x] Step 7.1.1: Implement Docling document parsing behind `DocumentParserPort`.
- [x] Step 7.1.2: Translate Docling output into Catalyst source records, observations, and evidence sets.
- [x] Step 7.1.3: Prevent Docling concrete types from entering internal layers.
- [x] Step 7.1.4: Add contract tests proving Docling is a boundary adapter, not an internal model.

### Task 7.2: Add PDF and document contradiction fixtures

- [x] Step 7.2.1: Add fixture for repeated page headers that look like headings.
- [x] Step 7.2.2: Add fixture for a table split across pages.
- [x] Step 7.2.3: Add fixture for OCR hyphenation artifacts.
- [x] Step 7.2.4: Verify coverage, table header preservation, rejection records, and projection versioning.

### Task 7.3: Prepare alternative document adapters

- [x] Step 7.3.1: Define adapter admission criteria for Unstructured, Haystack, and LlamaIndex.
- [x] Step 7.3.2: Require each adapter to pass the same port contract tests.
- [x] Step 7.3.3: Keep provider chunks as observations or candidates, never accepted Catalyst chunks by authority alone.

## Phase 8: Add Hierarchical Retrieval Context Recovery

### Task 8.1: Implement parent/child chunk structure

- [x] Step 8.1.1: Implement hierarchical candidate strategy.
- [x] Step 8.1.2: Implement `ParentChildProjection`.
- [x] Step 8.1.3: Preserve parent, child, previous, next, continuation, dependency, table, citation, code, summary, and retrieval-window relation kinds in the chunk graph.

### Task 8.2: Implement sentence-window recovery from ADR-0007

- [x] Step 8.2.1: Implement `SentenceWindowProjection`.
- [x] Step 8.2.2: Distinguish indexed chunk text from recovered context.
- [x] Step 8.2.3: Add tests proving retrieval context recovery does not rewrite accepted chunk identity.

### Task 8.3: Restrict blanket overlap

- [x] Step 8.3.1: Define evidence types that permit overlap, including pronoun, definition, citation, table continuation, speaker-turn, code import, and claim/example dependency.
- [x] Step 8.3.2: Reject fixed overlap as a default setting.
- [x] Step 8.3.3: Add benchmarks tracking index cost and retrieval quality with relation-based context recovery.

## Phase 9: Add Code Chunking With AST Evidence

### Task 9.1: Implement code AST observation from ADR-0005

- [x] Step 9.1.1: Implement `AstParserPort`.
- [x] Step 9.1.2: Add Tree-sitter or ast-grep adapter behind the port.
- [x] Step 9.1.3: Emit code observations for functions, classes, imports, blocks, declarations, and malformed syntax regions.

### Task 9.2: Implement AST code candidate formation

- [x] Step 9.2.1: Implement AST code candidate formation.
- [x] Step 9.2.2: Prefer AST-supported boundaries over line or token windows.
- [x] Step 9.2.3: Use recursive split/merge only when oversized code units require it.
- [x] Step 9.2.4: Produce repair or rejection records for syntax-breaking splits.

### Task 9.3: Implement code projection and fixtures

- [x] Step 9.3.1: Implement `CodeProjection`.
- [x] Step 9.3.2: Add relations for definitions, calls, imports, adjacency, and dependency context where admitted.
- [x] Step 9.3.3: Add contradiction fixtures for nested functions/classes and code fences that look like prose structure.

## Phase 10: Add Semantic Refinement Without Making It Primary

### Task 10.1: Implement semantic shift observation from ADR-0006

- [x] Step 10.1.1: Define semantic refinement policy flags and default them to optional or auto, not required.
- [x] Step 10.1.2: Implement `SemanticShiftObservation`.
- [x] Step 10.1.3: Add Sentence Transformers or Model2Vec adapter behind `EmbeddingPort`.
- [x] Step 10.1.4: Keep embeddings and semantic model outputs as evidence, not source truth.

### Task 10.2: Implement semantic refinement candidate strategy

- [x] Step 10.2.1: Implement `SemanticRefinementCandidateSet`.
- [x] Step 10.2.2: Allow semantic refinement only after structural candidates exist.
- [x] Step 10.2.3: Require evidence for weak or ambiguous structural regions before semantic refinement changes a boundary.
- [x] Step 10.2.4: Add tests proving baseline chunking works without embeddings or LLM calls.

### Task 10.3: Add semantic contradiction and reproducibility checks

- [x] Step 10.3.1: Record model identity and policy used for semantic observations.
- [x] Step 10.3.2: Add fixtures where semantic similarity would merge source-distinct regions incorrectly.
- [x] Step 10.3.3: Add audit output that distinguishes semantic evidence from structural evidence.

## Phase 11: Evaluate Candidate Quality And Strategy Tradeoffs

### Task 11.1: Implement candidate comparison

- [x] Step 11.1.1: Implement `compare-strategies` output with candidate sets, hard gate results, soft metrics, rejections, repairs, and selected graph.
- [x] Step 11.1.2: Include boundary clarity, chunk stickiness, context coherence, orphan count, repair count, semantic discontinuity, index cost estimate, and latency estimate.
- [x] Step 11.1.3: Add benchmark fixtures for every strategy admitted so far.

### Task 11.2: Implement retrieval sanity evaluation

- [x] Step 11.2.1: Define held-out retrieval sanity fixtures by source family.
- [x] Step 11.2.2: Compare paragraph grouping, recursive fallback, dynamic token sizing, hierarchy, AST code, and semantic refinement.
- [x] Step 11.2.3: Report quality and cost without letting benchmarks override hard invariants.

### Task 11.3: Expand source-family contradiction coverage

- [x] Step 11.3.1: Add legal clause fixture with nested numbering.
- [x] Step 11.3.2: Add transcript fixture with missing speaker labels.
- [x] Step 11.3.3: Add scientific paper fixture with citation-heavy paragraphs.
- [x] Step 11.3.4: Add definition/dependency fixture where “This means...” must stay near its referent.

## Phase 12: Add Specialized Modes Only After The Baseline Is Lawful

### Task 12.1: Admit late chunking only as a specialized mode

- [x] Step 12.1.1: Add a policy flag for late chunking that defaults to false.
- [x] Step 12.1.2: Require offline or premium indexing context before late chunking runs.
- [x] Step 12.1.3: Preserve lineage, projection versioning, and rejection visibility in late chunking outputs.

### Task 12.2: Admit LLM-guided candidate formation only as boundary-assisted formation

- [x] Step 12.2.1: Keep LLM providers behind boundary ports.
- [x] Step 12.2.2: Treat LLM output as candidate evidence, never as accepted structure by authority.
- [x] Step 12.2.3: Require audit records for prompt, policy, model identity, confidence, and rejected alternatives.

### Task 12.3: Keep chunk-free retrieval as research mode

- [x] Step 12.3.1: Create an experimental policy namespace for chunk-free retrieval.
- [x] Step 12.3.2: Prevent chunk-free experiments from weakening source, lineage, projection, or audit invariants.
- [x] Step 12.3.3: Require a new ADR before chunk-free retrieval becomes a primary Catalyst mode.
...
- [x] Step 13.2.1: Wire import checks, native-name checks, file-size checks, invariant tests, boundary contract tests, projection schema checks, and contradiction tests into CI.
- [x] Step 13.2.2: Ensure every invariant has unit tests.
- [x] Step 13.2.3: Ensure every boundary adapter has contract tests.
- [x] Step 13.2.4: Ensure every source family has contradiction fixtures.
- [x] Step 13.2.5: Ensure every strategy has benchmark fixtures.

### Task 13.3: Define the first release acceptance gate

- [x] Step 13.3.1: Prove a Markdown document can become an admitted chunk graph with retrieval and audit projections.
- [x] Step 13.3.2: Prove fixed-size slicing does not run on the default path.
- [x] Step 13.3.3: Prove every accepted chunk has source lineage.
- [x] Step 13.3.4: Prove every public projection has a schema version.
- [x] Step 13.3.5: Prove rejected candidates remain inspectable.
- [x] Step 13.3.6: Prove no concrete boundary adapter is imported inward.
